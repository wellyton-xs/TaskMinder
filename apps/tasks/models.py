...
class Task(models.Model):
    ...
    end_date = models.DateField('Prazo final', auto_now=False, auto_now_add=False)
    created_at = models.DateField(default=get_default_dia_de_criacao, editable=False)
    priority = models.CharField('Prioridade', max_length=1, choices=PRIORITY_CHOICES)
    category = models.ManyToManyField(Category, verbose_name='Categorias(opcional)', blank=True)
    completed = models.BooleanField(default=False)
    owner = models.ForeignKey(User, on_delete=models.CASCADE)

    def porcentagem_dias_passados(self):
        dias_passados = (timezone.now().date() - self.created_at).days
        total_dias = (self.end_date - self.created_at).days
    ...
    def enviar_email_tarefa_proxima(self):
        ...
        if dias_restantes <= 1:
            assunto = f"{self.owner.name}, a Tarefa {self.name}: Prazo Final Amanhã"
            mensagem = f"A tarefa {self.name} está próxima do prazo final de sua tarefa. Resta 1 dia."
